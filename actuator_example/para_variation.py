...
def run(material, files, ifResample = False):
    for file in files:
        k = file.stem
        ...
        relaxed_forces = sum(material.force(relaxed_coords)) / material.vertex_dual_length(relaxed_coords)
        data[k]["relaxed_forces"] = relaxed_forces
        
        return data, length_scale, material

def make_movie(
    file_stem, original_coords, relaxed_coords, relaxed_forces, fps: int = 30, dpi: int = 100, skip: int = 100, interactive: bool = False
):
    x_lim = np.array([np.min(original_coords[:, 0]), np.max(original_coords[:, 0])]) + [-padding, padding]
    y_lim = np.array([np.min(original_coords[:, 1]), np.max(original_coords[:, 1])]) + [-padding, padding]
    ...
    (line,) = ax.plot(relaxed_coords[0][:, 0], relaxed_coords[0][:, 1], color="r")
    relaxed_forces = np.sum(relaxed_forces[0], axis=0)
    ...
    def animate(i):
        line.set_data(relaxed_coords[i][:, 0], relaxed_coords[i][:, 1])
        time_text.set_text(time_template % (i))

        forces = np.sum(forces[i], axis=0)
        ...
        q.set_offsets(relaxed_coords[i])
    ...
    if interactive:
        ...
        with moviewriter.saving(fig, f"movies/{file_stem}.mp4", dpi=dpi):
...
if __name__ == "__main__":
    ...
    data, length_scale, material = run(mem, files, ifResample = True)
    make_movie(
    file_stem, original_coords, deformed_coords, forces, fps: int = 30, dpi: int = 100, skip: int = 100, interactive: bool = False
)